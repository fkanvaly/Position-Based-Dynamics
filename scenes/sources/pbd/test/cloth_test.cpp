...
void scene_model::create_rigid_body()
{
    ...
    cloth_parameter cloth_param; int N_cloth = 40; cloth_param.wind = 10;
    ...
    // scene_objects.add_fix_rigid_body( fix_rigid_body(SOLID, particules, scene_objects.No, mu_frcition(1,1), rigid_body::g_plane(50, r, origin, 1e-2), origin, color_LUT[ scene_objects.No%color_LUT.size() ] ) );

    scene_objects.add_cloth( cloth(particules, scene_objects.No, cloth_param, N_cloth, {0,0,0}, color_LUT[ scene_objects.No%color_LUT.size() ]));
    scene_objects.add_cloth( cloth(particules, scene_objects.No, cloth_param, N_cloth, {0,2,0}, color_LUT[ scene_objects.No%color_LUT.size() ]));
